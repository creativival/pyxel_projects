...
class APP:
    ...
    def __init__(self):
        ...
        self.x = 0
        self.y = -260
        self.pos_x = np.random.randint(0, 128, (20, 1))
        ...
        self.send_data_num = 10  # 送信するデータの数を少しずつ増やしていく（クラッシュ防止）
    ...
    def draw(self):
        ...
        for i in range(int(self.send_data_num)):  # 複数箇所に文字列を配置する
            for ii in range(20):  # 縦方向に文字を繋げる
                ...
                y = self.y + self.pos_y[i][0] + 6 * ii
                pyxel.text(x, y, str(self.random_strings[i][ii]), c)
        ...
        if pyxel.frame_count % 30 == 0:
            ...
            self.vox.send_data()
            self.vox.clear_data()
            # self.send_data_num = min(8, self.send_data_num + 0.1)
